...
### from https://github.com/jw9730/tokengt
...
class GraphFeatureTokenizer(nn.Module):
    ...
    def get_batch(node_feature, edge_index, edge_feature, node_num, edge_num, perturb=None):
        ...
        node_num = node_num.to(device=device, dtype=torch.long)[:, None] # [B, 1]
        edge_num = edge_num.to(device=device, dtype=torch.long)[:, None] # [B, 1]
    ...
    def get_index_embed(node_id, node_mask, padded_index):
        ...
        # print('padded_index',padded_index.shape)
    ...
    def forward(self, batched_data, perturb=None):
        (
            ...
            batched_data["node_feature"],
            batched_data["edge_feature"],
        ...
        if self.layer_norm:
            padded_feature = self.layer_norm(padded_feature)
